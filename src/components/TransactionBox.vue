  ...
  <q-list bordered separator class="full-height flex column no-wrap">
    ...
    <q-item class="q-pa-none" style="min-height: 57px">
      ...
      >
        ...
        <q-tab name="one2many" label="One2Many" />
        <q-tab name="many2one" label="Many2One" />
      </q-tabs>
    </q-item>
        ...
        <q-list bordered>
          ...
          >
            <template v-slot:header>
              ...
              <q-item-section caption side
                >ADA: {{ totalAmounts[i] }}</q-item-section
              >
            </template>
            <q-item v-for="nft in nfts" :key="nft.asset_name" class="row">
              ...
              <q-item-section>
                ...
                <q-item-label caption class="wallet-text">{{
                  nft.policy_id
                }}</q-item-label>
              </q-item-section>
    ...
    <q-item class="q-pa-none" v-if="selectedWallet.includes(true)">
      <q-item-section
        class="col-6 bordered q-pa-md flex column no-wrap"
        style="margin: 0"
      >
        <q-item-label caption><strong>ADA</strong> to send</q-item-label>
        <input
          type="number"
    ...
    <q-item class="column" style="min-height: auto">
      Receiving address:
      <q-input
        type="textarea"
    ...
    <q-btn size="40px" color="primary" :disable="receivingAddress === ''">
      SUBMIT
    </q-btn>
  </q-list>
</template>
